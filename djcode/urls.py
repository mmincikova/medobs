...
urlpatterns = patterns("djcode.reservations.views",
	...
	(r"^place/(?P<place_id>\d+)/(?P<for_date>\d{4}-\d{2}-\d{2})/$", "place_page"),
	...
	(r"^days_status/(?P<year>\d{4})/(?P<month>\d{2})/(?P<place_id>\d+)/$", "days_status"),
	(r"^booked/(?P<place_id>\d+)/(?P<for_date>\d{4}-\d{2}-\d{2})/$", "booked"),
