...
def create_visit_template(place, starttime, endtime, interval):
	...
	while templatetime.time() <= endtime.time():
		for day in Visit_template.DAYS:
			if day[0] <= 5: # do not create templates for Saturday and Sunday
				if not Visit_template.objects.filter(place=place, day=day[0], starting_time=templatetime.time()):
					print 'I: Creating template:  %s %s %s' % (place.name, day[1], templatetime.time())
					Visit_template.objects.create(place=place, day=day[0], starting_time=templatetime.time(), valid_since=TEMPLATE_VALID_SINCE)
				else:
					print 'W: Template already exists:  %s %s %s ... (skipping)' % (place.name, day[1], templatetime.time())
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		for place in Medical_office.objects.all():
			create_visit_template(place, starttime, endtime, interval)
